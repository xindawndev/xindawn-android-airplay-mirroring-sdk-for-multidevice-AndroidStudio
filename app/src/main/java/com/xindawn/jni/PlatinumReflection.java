...
public class PlatinumReflection {
	...
	public static final int MEDIA_RENDER_CTL_MSG_SIZECHANGED = (MEDIA_RENDER_CTL_MSG_BASE+12);
	...
	public static interface ActionReflectionListener{
		...
		public void audio_init(int id,int bits, int channels, int samplerate,int isaudio);
		public void audio_process(int id,byte data[],double timestamp,int seqnum);
		public void audio_destroy(int id);
	}
	...
	public static void audio_init(int id,int bits, int channels, int samplerate,int isaudio){
		if (mListener != null){
			mListener.audio_init(id,bits, channels, samplerate,isaudio);
		}
	}
	
	public static void audio_process(int id,byte data[],double timestamp,int seqnum){
		if (mListener != null){
			mListener.audio_process(id,data,timestamp,seqnum);
		}
	}
	
	public static void audio_destroy(int id){
		if (mListener != null){
			mListener.audio_destroy(id);
		}
	}
