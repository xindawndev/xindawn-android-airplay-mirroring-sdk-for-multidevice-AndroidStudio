...
public class MediaControlBrocastReceiver extends BroadcastReceiver{
	...
	private void TransdelControlCommand(Intent intent){
		...
		if (action.equalsIgnoreCase(MediaControlBrocastFactory.MEDIA_RENDERER_CMD_PLAY)){
			mMediaControlListener.onPlayCommand(intent.getStringExtra(MediaControlBrocastFactory.PARAM_CMD_PLAY));
		...
		else if (action.equalsIgnoreCase(MediaControlBrocastFactory.MEDIA_RENDERER_CMD_SIZECHANGED)){	
			mMediaControlListener.onSizeChangedCommand(intent.getStringExtra(MediaControlBrocastFactory.PARAM_CMD_SIZECHANGED));
		}
